...
class Softhand_Publisher(Node):
    ...
    def __init__(self):
        ...
        self.service = self.create_service(SetBool, 'Grasp_object', self.service_callback)
        self.object_grasping = None
    ...
    def timer_callback(self):
        ...
        if self.object_grasping:
            self.get_logger().info('Fingers moving to the grasping point... ...')
            self.publishing_bend_angle(self.bend_angle_array_r_point1)
        elif self.object_grasping is None:
            self.get_logger().info('Waiting for the robotic arm moving... ...')
        else:
            self.publishing_bend_angle(self.bend_angle_array_r_point0)
            self.get_logger().info('Fingers moving back to original releasing point... ...')
